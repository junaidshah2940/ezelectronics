...
class UserRoutes {
    ...
    initRoutes() {
         ...
         */
        this.router.get(
            "/roles/:role", this.authService.isLoggedIn, this.authService.isAdmin, 
        ...
        this.router.delete(
            ...
            (req: any, res: any, next: any) => this.controller.deleteUser(req.user, req.params.username)
                .then((bool: boolean) => res.status(200).end())
         ...
         */
        this.router.delete(
            "/",this.authService.isAdmin, this.authService.isLoggedIn,
