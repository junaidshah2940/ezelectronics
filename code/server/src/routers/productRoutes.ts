...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("arrivalDate").optional({ nullable: true }).isString().custom((value) => {
                if (value === '') {
                    return true
                }
        ...
        this.router.patch(
            ...
            body("quantity").isNumeric().isInt({ gt: 0 }),
            body("changeDate").optional({ nullable: true }).isString().custom((value) => {
                if (value === '') {
                    return true
                }
                return !isNaN(Date.parse(value)) && /^\d{4}-\d{2}-\d{2}$/.test(value)
        ...
        this.router.patch(
            ...
            param("model").isString().notEmpty(),
            body("quantity").isNumeric().isInt({ gt: 0 }),
            body("sellingDate").optional({ nullable: true }).isString().custom((value) => {
                if (value === '') {
                    return true
                }
                return !isNaN(Date.parse(value)) && /^\d{4}-\d{2}-\d{2}$/.test(value)
