import { User } from "../components/user";
import { Cart } from "../components/cart";
import CartDAO from "../dao/cartDAO";
import {CartNotFoundError, EmptyCartError, ProductNotInCartError} from "../errors/cartError";
import {EmptyProductStockError, LowProductStockError} from "../errors/productError";

/**
 * Represents a controller for managing shopping carts.
 * All methods of this class must interact with the corresponding DAO class to retrieve or store data.
 */
class CartController {
    private dao1: CartDAO

    constructor() {
        this.dao1 = new CartDAO
    }

    /**
     * Adds a product to the user's cart. If the product is already in the cart, the quantity should be increased by 1.
     * If the product is not in the cart, it should be added with a quantity of 1.
     * If there is no current unpaid cart in the database, then a new cart should be created.
     * @param user - The user to whom the product should be added.
     * @param productId - The model of the product to add.
     * @returns A Promise that resolves to `true` if the product was successfully added.
     */
    async addToCart(user: User, product: string)/*: Promise<Boolean>*/ {
        return new Promise<Boolean>((resolve, reject) => {
            try {
                this.dao1.getCart(user)
                    .then((cart) => {
                        this.dao1.checkProduct(product)
                            .then((addedProduct) => {
                                if(cart.products.some((product) => product.model == addedProduct.model)){
                                    this.dao1.updateCart(user, addedProduct.model, 1)
                                        .then((result) => {
                                            resolve(result)
                                        })
                                        .catch((error) => {
                                            reject(error)
                                        })
                                }else{
                                    this.dao1.insertCart(user, addedProduct.model)
                                        .then((result) => {
                                            resolve(result)
                                        })
                                        .catch((error) => {
                                            reject(error)
                                        })
                                }
                            }).catch((error) => {
                                  reject(error)
                        })
                    })
            } catch (error) {
                reject(error)
            }
        })
    }


    /**
     * Retrieves the current cart for a specific user.
     * @param user - The user for whom to retrieve the cart.
     * @returns A Promise that resolves to the user's cart or an empty one if there is no current cart.
     */
    async getCart(user: User)/*: Cart*/ {
        return new Promise<Cart> ((resolve, reject) => {
            try {
                this.dao1.getCart(user)
                    .then((cart) => {
                        resolve(cart)
                    })
                    .catch((error) => {
                        reject(error)
                    })
            }catch (error) {
                reject(error)
            }
        })
    }

    /**
     * Checks out the user's cart. We assume that payment is always successful, there is no need to implement anything related to payment.
     * @param user - The user whose cart should be checked out.
     * @returns A Promise that resolves to `true` if the cart was successfully checked out.
     * 
     */
    async checkoutCart(user: User) /**Promise<Boolean> */ {
         return new Promise<Boolean>((resolve, reject)  => {
            try {
                this.dao1.getCart(user)
                    .then((cart) => {
                        if(cart.products.length == 0){
                            reject(new CartNotFoundError())
                            return
                        }
                        const promises :Promise<Boolean>[] = []
                        cart.products.forEach((product) => {
                            promises.push(new Promise<Boolean>((resolve,reject) => {
                            this.dao1.checkProduct(product.model)
                             .then((product1) => {
                                if (product1.quantity < product.quantity) {
                                    reject(new LowProductStockError())
                                }
                                 resolve(true)

                             }).catch((error) => {
                                reject(error)
                            })
                            }))
                        })
                         Promise.all(promises)
                             .then(() => {
                            cart.products.forEach((product) => {
                                this.dao1.updateQuantity(product.model, product.quantity)
                                    .then((result) => {
                                        if (result == false) {
                                            reject(new LowProductStockError())
                                        }
                                    }).catch((error) => {
                                    reject(error)
                                })
                            })
                        }).then(() => {
                            this.dao1.checkoutCart(user)
                                .then((result) => {
                                    resolve(result)
                                }).catch((error) => {
                                    reject(error)
                            })
                        }).catch((error) => {
                                reject(error)
                                return
                            })
                    })
            } catch (error) {
                reject(error)
            }
        })
    }

    /**
     * Retrieves all paid carts for a specific customer.
     * @param user - The customer for whom to retrieve the carts.
     * @returns A Promise that resolves to an array of carts belonging to the customer.
     * Only the carts that have been checked out should be returned, the current cart should not be included in the result.
     */
    async getCustomerCarts(user: User) {
        return new Promise<Cart[]>((resolve, reject) => {
            try {
                const carts: Cart[] = []
                this.dao1.getCartsID(user, true)
                    .then((ids) => {
                        const promises :Promise<Cart[]>[] = []
                        ids.forEach((id) => {
                            promises.push(new Promise<Cart[]>((resolve, reject) => {
                                this.dao1.getCartProducts(user.username, id)
                                    .then((cart) => {
                                        carts.push(cart)
                                        resolve(carts)
                                    }).catch((error) => {
                                        reject(error)
                                })
                            }))
                        })
                        Promise.all(promises)
                            .then(() => {
                                resolve(carts)
                            }).catch((error) => {
                                reject(error)
                        })
                    })
                    .catch((error) => {
                        reject(error)
                    })
            } catch (error) {
                reject(error)
            }
        })
    } /**Promise<Cart[]> */

    /**
     * Removes one product unit from the current cart. In case there is more than one unit in the cart, only one should be removed.
     * @param user The user who owns the cart.
     * @param product The model of the product to remove.
     * @returns A Promise that resolves to `true` if the product was successfully removed.
     */
    async removeProductFromCart(user: User, product: string) /**Promise<Boolean> */ {
        return new Promise<Boolean>((resolve, reject) => {
            try {
                this.dao1.getCart(user)
                    .then((cart) => {
                        if(cart.products.length === 0){
                            reject(new CartNotFoundError())
                        }
                        if(!cart.products.some((product1) => product1.model == product)){
                            reject(new ProductNotInCartError())
                        }
                        new Promise((resolve, reject) => {
                            this.dao1.checkProduct(product)
                                .then((product1) => {
                                    resolve(true)
                                    }
                                ).catch((error) => {
                                    if(error instanceof EmptyProductStockError) {
                                        resolve(true)
                                    } else {
                                    reject(error)
                                }
                            })
                        }).then(() => {
                            if(cart.products.some((product1) => product1.model == product && product1.quantity == 1)){
                                this.dao1.removeFromCart(user, product)
                                    .then((result) => {
                                        resolve(result)
                                    }).catch((error) => {
                                    reject(error)
                                })
                            }else{
                            this.dao1.updateCart(user, product, -1)
                                .then((result) => {
                                    resolve(result)
                                }).catch((error) => {
                                reject(error)
                            })}
                        }).catch((error) => {
                            reject(error)
                        })
                    })
                    .catch((error) => {
                        reject(error)
                    })
            } catch (error) {
                reject(error)
            }
        })
    }


    /**
     * Removes all products from the current cart.
     * @param user - The user who owns the cart.
     * @returns A Promise that resolves to `true` if the cart was successfully cleared.
     */
    async clearCart(user: User)/*:Promise<Boolean> */ {
        return new Promise<Boolean>((resolve, reject) => {
            try {
                this.dao1.getCartsID(user, false)
                    .then((ids) => {
                        this.dao1.getCartProducts(user.username, ids[0])
                            .then((cart) => {
                                if(cart.products.length == 0){
                                    reject (new CartNotFoundError())
                                }
                            })
                            .then(() => {
                                this.dao1.clearCart(ids[0])
                                    .then((result) => {
                                        resolve(result)
                                    }).catch((error) => {
                                    reject(error)
                                })
                            }).catch((error) => {
                            reject(error)
                        })
                    }).catch((error) => {
                    reject(error)
                })
            } catch (error) {
                reject(error)
            }
        })
    }

    /**
     * Deletes all carts of all users.
     * @returns A Promise that resolves to `true` if all carts were successfully deleted.
     */
    async deleteAllCarts() /**Promise<Boolean> */ {
        return new Promise<Boolean>((resolve, reject) => {
            try {
                this.dao1.deleteAllCarts()
                    .then((result) => {
                        resolve(result)
                    })
                    .catch((error) => {
                        reject(error)
                    })
            } catch (error) {
                reject(error)
            }
        })
    }

    /**
     * Retrieves all carts in the database.
     * @returns A Promise that resolves to an array of carts.
     */
    async getAllCarts() /*:Promise<Cart[]> */ {
        return new Promise<Cart[]>((resolve, reject) => {
            try {
                class Tuple {
                    id: any;
                    username: string;

                    constructor(id: any, username: string) {
                        this.id = id;
                        this.username = username;
                    }
                }
                const carts: Cart[] = []
                this.dao1.getAllCarts()
                    .then((tuples) => {
                        const promises: Promise<Cart[]>[] = []
                        tuples.forEach((tuple) => {
                            promises.push(new Promise((resolve, reject) => {
                                this.dao1.getCartProducts(tuple.username, tuple.id)
                                    .then((cart) => {
                                        if(cart.products.length == 0){
                                            resolve(carts)
                                        } else {
                                            carts.push(cart)
                                            resolve(carts)
                                        }
                                    }).catch((error) => {
                                    reject(error)
                                })
                            }))
                        })
                        Promise.all(promises)
                            .then(() => {
                                resolve(carts)
                            })
                            .catch((error) => {
                                reject(error)
                            })
                    }).catch((error) => {
                    reject(error)
                })
            }catch (error) {
                reject(error)
            }
        })
    }
}

export default CartController